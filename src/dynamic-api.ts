...
export class DynamicApi {
    ...
    for (const [name, endpoint] of Object.entries(this.config.endpoints)) {
      ...
      if (endpoint.rateLimitPerSecond) {
        this.rateLimiters.set(
          name,
          new RateLimiter({
            tokensPerInterval: endpoint.rateLimitPerSecond,
            interval: "second",
          }),
        );
      }
   ...
   * @internal
   */
  private createMethod<T extends Record<string, any>>(
    name: string,
    ...
    return async (
      ...
      return this.requestQueue.enqueue(() =>
        this.makeRequest(endpoint, data, config)
      );
    };
  }
  ...
  private async makeRequest<T extends Record<string, any>>(
    ...
    for (const param of params) {
      ...
      if (data[paramKey] !== undefined) {
        ...
        if (url.includes(`:${paramKey}`)) {
          url = url.replace(
            `:${paramKey}`,
            encodeURIComponent(data[paramKey] as string | number | boolean),
          );
        } else {
    ...
    const finalRetryConfig: RetryConfig = {
      maxRetries: retryConfig?.maxRetries ||
        this.config.globalRetryConfig?.maxRetries || 3,
      retryDelay: retryConfig?.retryDelay ||
        this.config.globalRetryConfig?.retryDelay || 1000,
      retryCondition: retryConfig?.retryCondition ||
        this.config.globalRetryConfig?.retryCondition,
    };
